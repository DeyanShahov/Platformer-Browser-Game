...
// Key press tracking variables for skill trees (moved from game.js Phase 3)
...
// Key press tracking variables for character stats (moved from game.js Phase 3)
...
// Global variables for skill tree timing (moved from game.js Phase 3)
...
// Input coordination functions (moved from game.js Phase 3)
function handleSkillTreeKeys(gameState, keys, lastSkillTreeToggleTime) {
  ...
  //console.log('[DEBUG] Keys - 5:', keys['5'], '6:', keys['6'], '7:', keys['7'], '8:', keys['8']);
  ...
  key5Pressed = keys['5'];
  if (key5Pressed && !key5WasPressed && gameState.players && gameState.players.length >= 1) { // Key just pressed
...
function handleCharacterStatsKeys(gameState, keys, lastSkillTreeToggleTime) {
  const now = performance.now();
  ...
  //console.log('[DEBUG] Character stats keys - 9:', keys['9'], '0:', keys['0'], '-:', keys['-'], '=', keys['=']);

  // Player 1 character stats (key 9) - toggle player's own stats
  key9Pressed = keys['9'];
  if (key9Pressed && !key9WasPressed && gameState.players && gameState.players.length >= 1) { // Key just pressed
...
// Input coordination functions (moved from game.js Phase 3)
window.MenuSystem = window.MenuSystem || {};
window.MenuSystem.handleSkillTreeKeys = handleSkillTreeKeys;
window.MenuSystem.handleCharacterStatsKeys = handleCharacterStatsKeys;
window.MenuSystem.lastSkillTreeToggleTime = lastSkillTreeToggleTime;
